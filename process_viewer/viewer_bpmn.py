...
class BPMNFlowProcessViewer:
    ...
    def render_to_dot(self, depth: int) -> graphviz.Digraph:
        """Render the process tree at a given depth and return Graphviz object."""
        g = graphviz.Digraph(format="png")
        g.attr(rankdir="LR")
        g.attr("node", fontname="Helvetica")

        g.node("START", "Start", shape="circle", style="filled",
            fillcolor="#4caf50", fontcolor="white", width="0.6")
        g.node("END", "End", shape="doublecircle", style="filled",
            fillcolor="#f44336", fontcolor="white", width="0.6")

        root_s, root_e = self._add_to_graph(g, self.tree, 0, depth)
        g.edge("START", root_s)
        g.edge(root_e, "END")
